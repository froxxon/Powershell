...
Function Invoke-ElasticSearchRequest {
    [CmdletBinding()]
    param(
        [ValidateSet("Tls","Tls11","Tls12")]$UseSecurityProtocol,
    ...
    #region SecurityProtocol conditions
        if($UseSecurityProtocol){
            [System.Net.ServicePointManager]::SecurityProtocol = [Net.SecurityProtocolType]::$($UseSecurityProtocol)
            Write-Verbose "Using security protocol: $UseSecurityProtocol"
            if($ElasticSearchUri -match '^http:'){
                $ElasticSearchUri = $ElasticSearchUri -replace 'http:','https:'
                Write-Verbose "Switching from HTTP to HTTPS in ($ElasticSearchUri)"
            }
        }
        else{
            if($ElasticSearchUri -match '^https:'){
                [System.Net.ServicePointManager]::SecurityProtocol = [Net.SecurityProtocolType]::Tls12
                Write-Verbose "HTTPS uri was used but no SecurityProtocol specified, using Tls12"
            }
        }
    #endregion
    ...
    #region Using Credentials in Header parameter
        if($Credential){
            Write-Verbose "Building a header with username/password credentials"
            $userString = $Credential.UserName
            $passString = $((New-Object PSCredential "user",$Credential.Password).GetNetworkCredential().Password)
            $credPair = "$($userString):$passString"
            $encodedCredentials = [System.Convert]::ToBase64String([System.Text.Encoding]::ASCII.GetBytes($credPair))
            $headers = @{ Authorization = "Basic $encodedCredentials" }
            [void]$RequestParams.Add("Headers",$headers)
        }
    #endregion
